...
off the unresolved-marker census, **3,265 across 81 documents** after round 37 — **round 41
took 704 off it from one document**, and **row 14** below is the last ranked row. **Nine of the thirteen hits were
...
**Since round 36 the board has carried no register-bearing row, so rows 13 and 14 came off
the UNMEASURED surface** — the unresolved-marker census, which no invariant watches. It stood
at **3,265 across 81 documents** after round 37 (acts 1,900 / rules 934 / ordinance 431).
**Row 14 is closed by round 42 and row 13 by round 41, so the ranking below holds no open
measured row**; the next round comes off the q-rows or a fresh census. Neither row was a
hypothesis — but each was wrong about its own mechanism, and both corrections are in the
Result cells: row 13's "page FOLIO" is another regime's body, and row 14's "untraced"
CHAPTER VIII is printed on p21 with the same defect as CHAPTER XIV.

| # | pick this up | hits | the single blocker | plan.md |
|---|---|---|---|---|
| ~~13~~ (r41) | ~~**Sales Tax Rules 2006 (01-01-2025): 869 unresolved markers, 0 notes**~~ | **761 → 57; 0 → 367 records, 321 bound** | **CLOSED 2026-09-18 (round 41), and the ledger's diagnosis was half wrong.** It said 11.04pt is the page FOLIO. A folio is one word per page; this is **1,168 words, 525 of them on p31 alone** — it is *another regime's BODY*. The document sets its rules at 12.0 and prints two whole pages (p31, p152) at 11.0, so the fit returned 12.0/11.0, a gap of 1.0 under `SIZE_GAP_MIN`, and `calibrate` gave the DOCUMENT up. **The gap test was already the right discriminator, consulted in the wrong place**: reaching it means only that this CANDIDATE is not a footnote size, so skip it and take the next that clears the gap. Two guards decide, and each was measured against the document the other misses — **mass** (≥1% of sampled words; Finance Act 2022 offers 5.5pt at 12 words) and **position** (≥50% of its words in the bottom 40% of the page; **Finance Act 2024 sets 573 words at 8.0pt that are SCHEDULE TARIFF ROWS**, and shipping without this guard was measured minting 10 false records reading `S. No. Taxable Income Rate of Tax`). Eight candidates are rejected by the floor alone and four by the position test alone. **Exactly ONE document in the corpus changes**; four controls are byte-identical and the other 136 acts+rules sources are accounted for by measuring `(body_size, footnote_size)` on all 141 before and after — 140 identical. **The ledger's prescription, "fix the sampling", was measured and is wrong**: restricting the histogram to body-dominant pages moves 32 documents, costs Finance Act 2019 its zone, and hands Customs' second slot to 8.0pt — its MARKER size — because Customs prints its notes on whole collector pages (p59 is 746 words, all 9.0pt). **⚠ The rider did NOT fire**: uppercase section codes inside a `<sup>` are **0 before and 0 after**, because `upper_ok` also requires `size < footnote_size` and the codes print at 9.0pt, which is not strictly smaller than the corrected 9.0. Correcting `footnote_size` closes the rider by itself. `<table>` 64→39 is a fix: leaf 13's three page-split fragments merge into **one 69-row table**. Cost stated: **3 of 367 records are wrong**, all `^cont` on leaf 165's annexed forms | [artifact](../wip/phase3-round41-second-body-calibration.md) |
| ~~14~~ (r42) | ~~**Customs Rules 2001: three CHAPTER captions missing from the tree**~~ | **41 → 43 chapters; two of three recovered, the third refused on evidence** | **CLOSED 2026-09-18 (round 42), and it was TWO causes, not three.** The row said CHAPTER VIII was untraced and might be a source omission. **Its caption is printed, on p21, and it is the same defect as XIV**: `41&46 [CHAPTER VIII` — VII and VIII start on the SAME page, which is why the row thought nothing was printed between VII and IX. `builder._STRUCT_DECOR_RE` knew only whitespace between stacked markers, so it read ONE marker and stopped at the `&`. **`grammar.MARKER_PREFIX` has admitted `,` and `&` since the round that measured them** and `_MARKER_RUN_SEP_RE` is exactly `\s*[,&/]\s*` — this was a second, narrower implementation of the same idea, and the suite carried a third. All three now agree. Measured: chapters **41 → 43**, leaves **1,105 → 1,106** (rule **89** recovered, previously glued to its neighbour), re-parenting exact (XIII's 33 → 12 + XIV's 21; VII's 19 → 3 + VIII's 17), tables and rows **unchanged**, body words −8 (the captions moving into container nodes). **Citations −4 are all spurious and round 37 predicted them**: `561.2` was bound to leaf **325** and `561.41`/`561.46` to leaf **88** — the leaves that were swallowing the captions, exactly the `u1b` residue. **⚠ CHAPTER XX is deliberately NOT recovered.** It prints `“Chapter XX`, and admitting the quote was measured first: **the same glyph opens QUOTED REPEALED TEXT**. Customs Act 1969 p219 prints `“Chapter XIX-A` under *“At the time of omission section 196-K to 196-U were as under:”* while the LIVE CHAPTER XIX-A is already cut from `1 [ CHAPTER XIX-A` sixteen lines above — accepting it mints a duplicate chapter and re-parents eleven repealed sections as law, in at least twenty Customs editions. The suite refuses the same line via `_QUOTE_CUE`. Both are pinned in `NOT_BOUNDARIES`. The row's *“rules 127-132 are absent”* is true and **unrelated**: the source prints them only as numbered entries in the terminal `As Amended:-` S.R.O. list | [artifact](../wip/phase3-round42-marker-run-decoration.md) |
...
| 12 | the OCR decision | — | **DECIDED 2026-09-04: out of scope, deliberately.** `data/ocr_cache` stays 0 B. Not work — the decision is the deliverable, and it is recorded. **2026-09-24: one scoped exception** — the four shortlisted scanned rules documents are ingested from an offline, voted vision sidecar ([vision-ocr-before-after.md](vision-ocr-before-after.md)); the Tesseract decision for the rest of the corpus is unchanged and `data/ocr_cache` is still 0 B | [Phase 2](plan.md#phase-2--the-ocr-half-a-decision-not-work) |
...
| q4 | **THIRD SCHEDULE Table-I still renders as prose** | **open — SIZED by round 43, not closed** | **The trace is confirmed and the row is right that there is nothing to widen.** Sized 2026-09-18: `_is_header_start` fires **1,162** times across the 143 acts+rules sources; **836 are accepted today** (a numbering row follows within 8 lines) and **326 are rejected across 59 documents** — 286 `S.No.` headers, 40 the bare keyword `TABLE`, acts 170 / rules 156. **326 is what any relaxation admits: 28% of every header start in the corpus.** A deliberately strict positive test — header with **≥3 x0 clusters** AND **≥4 of the next 8 lines carrying columns** — still leaves **213 sites across 51 documents** (31 in Customs Rules 2001, 11 each in five Federal Excise editions and Finance Act 2021). Round 39's `Col.(N)` change moved **17** documents and needed a two-pass conversion of 119 sources; this is three times that reach, with no numbering row to validate the recovered column count against. **A SECOND defect the row does not carry:** once a span is accepted, `find_table_spans` ends it on `_NUM_TOKEN` (`^(?:Col\.?\s*)?\(\d+\)$`), which matches `(1)` and `(2)` — *the Third Schedule's own serials* — so relaxing the numbering-row requirement alone yields a **one-row table**. It is two changes. Recommended staging: infer columns from the **data's** left-edge clusters (138.7 / 175.1 / 409.6 on p90), not the header's, which wraps its third column over two more lines and does not align; fix the terminator in the same change; budget a round-39-scale measurement | [sizing](../wip/phase3-round43-q4-table-span-sizing.md) |
| ~~q6~~ (r44) | ~~**Finance Act 2019 ships 95 false footnote records**~~ | **95 → 0, and no body text moves** | **CLOSED 2026-09-18 (round 44).** Found during round 41 and deferred as *“worth its own row”*. The document sets its schedules' tariff tables at **8.0pt against an 11.0pt body**, so the size split is textbook and everything under it is *table*: 95 records of which **5 bound to anything and none read like a note** (`Breeding bulls 0102.2910 0% Nil`; one record swallowed the table's own numbering row). **The discriminator was already in the tree** — `pagemodel._is_amendment_note` documents *“97.7% of real footnotes match; a body rate/penalty TABLE cell never does”* — but it is consulted per marker line and never about the zone as a whole. **Two other rules were measured first and both were destructive:** round 41's position test on the primary fit demotes **38 documents, 26 staged, including every Customs Act edition at 31-44%** (Customs prints its notes on whole collector pages, where there is no body to sit below — that test is a PROMOTION guard, not a demotion rule), and a per-page *“footnote starts below body”* test still demotes **9 staged documents whose records are 92-100% real notes**. The shipped rule is `zone_mode "size"` AND **≥100 zone lines** AND **<2% carrying an edit verb**. **The line floor is what protects Customs Rules 2001**, whose apparatus is printed once at the end at BODY size (round 37): its size zone holds **24 lines, none a note**, while the document carries **419 records and 656 citations**. Demotes **2 documents, 1 staged**; both thresholds sit in a wide gap (24 vs 100; 0.2% vs the next document at 5.4%). **Nothing is lost — the 95 records were duplication**: `Cocoa powder` 9 → 2, `Breeding bulls` 4 → 2, and leaves, tables, rows and body words are all unchanged. Three controls byte-identical. **No invariant saw it either way** — `run_suite.py acts` is 65/65 with 0 hits before and after | [artifact](../wip/phase3-round44-zone-must-hold-notes.md) |
...
**Result** — _(partial, 2026-09-24)_ A scoped route exists now: `tools/vision_ocr.py` votes
three vision models offline into a `<pdf>.vision.json` sidecar that `ocr_page` reads
deterministically. It ingested ITR 2002 (24.11.2023), PSW Deputation 2021 and both Sales
Tax Rules editions: [vision-ocr-before-after.md](vision-ocr-before-after.md). The rest of
the scanned corpus has no sidecar and behaves exactly as before.
