...
> **Seeded from the Acts suite.** `packages/rules_ingest` is a fork of
> `packages/acts_ingest`, so its regression suite starts as a copy of
> `tools/acts/suite/` and diverges from there. An invariant that turns out to be
> Acts-specific should be deleted here rather than weakened, and a Rules-specific one
> (4-digit rule codes, a folio with no support, a contents page parsed as body) added.
>
> The runner is [`tools/rules/run_tests.py`](../run_tests.py) and the corpus it
> defaults to is `$CORPUS_RULES` (`data/corpora/rules/`).
