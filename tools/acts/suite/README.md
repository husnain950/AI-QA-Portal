...
> **Vendored.** Copied from the standalone pipeline repository (`CC-FBR/Acts_fbr/tests/`), which is
> not part of this monorepo. Paths below are written for that layout; here the runner is
> [`tools/acts/run_tests.py`](../run_tests.py) and the corpus it defaults to is `$CORPUS_ACTS`
> (`data/corpora/acts/`), not `./output/`. The package is imported as `suite`, not
