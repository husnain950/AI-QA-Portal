...
def _default_jsons() -> list[str]:
    """Every JSON in the CORPUS_ORDINANCE corpus -- the bare command gates ALL editions.

    (It used to test only whichever file sorted first, which silently gated
    the 30.06.2024 edition and nothing else.)
    """
    root = os.environ.get("CORPUS_ORDINANCE") or os.path.join(
        _ROOT, "data", "corpora", "ordinance"
    )
    # .env ships CORPUS_ORDINANCE as a repo-relative path, so anchor it; join leaves
...
def main(argv=None) -> int:
    ...
    if args.pdf:
        from fbr_ingest import run as convert
