...
Within the footnote zone (already separated by :mod:`fbr_ingest.pagemodel`)
...
is assembled later in :mod:`fbr_ingest.builder`, which knows the printed-page
offset.
"""
...
def ref_sort_key(ref: str):
    ...
    if marker.isdigit():
        return (page_n, 1, int(marker), marker)
    return (page_n, 0, 0, marker)
...
def _is_marker_word(w, n_words_on_line: int = 99) -> bool:
    ...
    if not (t == "*" or t.isdigit()):
        return False
    return w.size <= 7.8 or n_words_on_line == 1
...
def parse_footnotes(footnote_lines, grid_boxes=()) -> list[Footnote]:
    ...
    for ln in footnote_lines:
        ...
        if not words:
            continue
        first = words[0]
        is_cand = _is_marker_word(first, len(words)) and first.x0 <= LEFT_MARGIN_MAX
        if is_cand and _accept_marker(first.text.strip(), words[1:],
                                      accepted, bool(lead)):
            rest = words[1:]
            current = {"marker": first.text.strip(), "lines": []}
...
def merge_footnote_continuations(page_footnotes: dict) -> None:
    ...
    for pg in sorted(page_footnotes):
        ...
        if fns and fns[0].marker == CONT_MARKER:
            cont = fns.pop(0)
            target = _continuation_target(last_fns)
